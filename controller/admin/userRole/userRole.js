...
})
{
  ...
  const softDeleteManyUserRole = async (ids, loggedInUser) => {
    try {
      if (ids){
        ...
        let data = await userRoleService.bulkUpdate(query, updateBody);
  ...
  const deleteManyUserRole = async (data, loggedInUser) => {
    try {
      if (data && data.ids){
        let ids = data.ids;
        const query = { '_id':{ '$in':ids } };
  ...
  const softDeleteUserRole = async (id,loggedInUser)=>{
    try {
      if (id){
        const query = { _id:id };
        const updateBody = { isDeleted: true, };
        let updatedUserRole = await userRoleService.softDeleteByQuery(query, updateBody);
  ...
  const deleteUserRole = async (data,id,loggedInUser) => {
    try {
      if (id){
        const query = { _id:id };
        let deletedUserRole = await userRoleService.findOneAndDeleteDocument(query);
