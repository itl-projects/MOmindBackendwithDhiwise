...
function makeAuthController ({
  authService,makeUniqueValidation,adminService,userTokenService,makeAdmin,roleService,userRoleService
}){
  const register = async ({ data }) => {
    try {
      ...
      const user = makeAdmin(data, 'insertAdminValidator');
      ...
      const result = await adminService.createDocument(user);
  ...
  const forgotPassword = async (params) => {
    try {
      ...
      let user = await adminService.getSingleDocumentByQuery(where);
      if (user) {
        let {
  ...
  const validateResetPasswordOtp = async (params) => {
    try {
      ...
      let user = await adminService.getSingleDocumentByQuery({ 'resetPasswordLink.code': params.otp });
  ...
  const resetPassword = async (params) => {
    try {
      ...
      let user = await adminService.getSingleDocumentByQuery({ 'resetPasswordLink.code': params.code });
      if (user && user.resetPasswordLink.expireTime) {
